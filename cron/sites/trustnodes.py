...
class CTrustnodes(Main):
    ...
    def start(self):
        try:
            menu = self.get_menu('id', 'menu-mobile-menu')
    ...
    def get_news(self, url, section):
        ...
        wrapper = soup.find('section', {'class': 'dt-block'})
        ...
        for block in blocks:
            h3 = block.find('h3', {'class': 'news-title'})
            ...
            title = a.text.strip()
            date = block.find('span', {'class': 'article-date'}).text.strip()

            if title in self.posts:
                continue

            if self.check_date(date, False, "%B %d, %Y %I:%M %p") is None:
                break
