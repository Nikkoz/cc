...
class CForklog(Main):
    ...
    def start(self):
        try:
            menu = self.get_menu('id', 'menu-header2')
    ...
    def get_news(self, url, section):
        ...
        wrapper = soup.find('main', {'id': 'category_page'})

        try:
            blocks = wrapper.find_all('article', {'class': 'item'})
        except AttributeError:
            raise RuntimeError("structure of the news list has changed")

        for block in blocks:
            url = self.check_url(block.find('a').get('href'))
            date = block.find('article_date').text

            span = block.find_all('a')[1].find('span')
            ...
            if self.check_date(date, False, "%d.%m.%Y") is None:
                break
    ...
    def get_post(self, url):
        ...
        content = soup.find('section', {'id': 'article_content'})
